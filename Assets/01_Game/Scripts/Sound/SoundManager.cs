...
namespace Assets.AT
{
    public class SoundManager : MonoBehaviour
    {
        ...
        [SerializeField] private SoundDataWrapper[] _soundDataArray;// インスペクターから設定されるサウンドデータの配列です。

        [System.Serializable]
        public class SceneBGM // シリアライズ可能なシーンごとの BGM 設定クラスです
        {
            public string sceneName;
            public string bgmName;
        }
        [SerializeField] private SceneBGM[] _inScenePlay; // インスペクターから設定されるシーンごとの BGM 設定の配列です。

        [Header("AudioSource プール関連設定")]
        [SerializeField, Tooltip("SoundManagerが自身で保持するAudioSourceの初期数（UI/System SE用）")]
        private int _initialManagerAudioSources = 20;

        [SerializeField, Tooltip("各ゲームオブジェクトが保持できるAudioSourceの最大数")]
        ...
        private void Awake() // シングルトンの初期化、AudioSourceプールの設定、サウンドデータとシーンBGMのマッピング、ミキサーグループのキャッシュ初期化
        {
            // シングルトンの設定
            if (Instance == null)
            {
            ...
            foreach (var soundDataWrapper in _soundDataArray)
            {
                if (soundDataWrapper.soundData != null)
                {
                    // 重複登録を避けます。
                    if (!_soundDictionary.ContainsKey(soundDataWrapper.name))
                    {
                        _soundDictionary.Add(soundDataWrapper.name, soundDataWrapper.soundData);
                    }
                    else
                    {
        ...
        private void Start()
        {
            ...
            this.UpdateAsObservable()
                .Where(x => (_currentBGMSource != null && _currentBGMSource.isPlaying && _currentSoundData is LoopSoundData))
                .Subscribe(_ =>
                {
                    LoopCheck(_currentBGMSource, _currentSoundData);
                })
                .AddTo(this);
        }

        private void OnDestroy() // シーンロードイベントの解除、フェード処理中の購読解除、および全てのAudioSourceプールのクリーンアップ
        {
            if (Instance == this)
            {
        ...
        public void StopBGMWithFade(float fadeDuration)
        {
            ...
            _bgmFadeDisposable = Observable.Interval(TimeSpan.FromSeconds(Time.deltaTime))
                .TakeWhile(_ => _currentBGMSource != null && _currentBGMSource.volume > 0)
                .Subscribe(
                    _ =>
                    {
                        if (_currentBGMSource != null)
                        {
                            _currentBGMSource.volume -= startVolume * (Time.deltaTime / fadeDuration);
                            if (_currentBGMSource.volume <= 0)
                            {
                                _currentBGMSource.Stop();
                                _currentBGMSource.volume = startVolume; // 元のボリュームに戻しておきます。
        ...
        public void StopBGMByNameWithFade(string bgmName, float fadeDuration)
        {
            ...
            string playingBGMName = _soundDictionary.FirstOrDefault(x => x.Value == _currentSoundData).Key;
        ...
        public void PlayBGM(string bgmName, string mixerGroupName = null, float fadeInDuration = 0f)
        {
            ...
            _currentBGMSource = GetUnusedManagerAudioSource();
            if (_currentBGMSource == null)
            {
            ...
            ResetAudioSourceProperties(_currentBGMSource); // プロパティを初期状態にリセットします。
            _currentBGMSource.clip = soundData.GetAudioClip();
            _currentBGMSource.loop = true;
            _currentBGMSource.spatialBlend = 0f; // BGMは通常2Dサウンドです。

            SetAudioMixerGroup(_currentBGMSource, mixerGroupName);

            if (fadeInDuration > 0f)
            {
                _currentBGMSource.volume = 0f; // フェードイン開始時はボリュームを0にします。
                _currentBGMSource.Play();

                _bgmFadeDisposable = Observable.Interval(TimeSpan.FromSeconds(Time.deltaTime))
                    .TakeWhile(_ => _currentBGMSource != null && _currentBGMSource.volume < 1f)
                    .Subscribe(
                        _ =>
                        {
                            if (_currentBGMSource != null)
                            {
                                _currentBGMSource.volume += (1f / fadeInDuration) * Time.deltaTime;
                                if (_currentBGMSource.volume >= 1f)
                                {
                                    _currentBGMSource.volume = 1f; // 最大ボリュームに固定します。
            ...
            {
                _currentBGMSource.volume = 1f; // フェードインしない場合は即座に最大ボリュームを設定します。
                _currentBGMSource.Play();
            }
        }
